#!/bin/bash
FILES_OLD="/usr/share/cyware-indexer/plugins/opensearch-security/securityconfig"
FILES_NEW="/etc/cyware-indexer/opensearch-security"
declare -A files_old
declare -A files_new
PACKAGE_NAME="${1}"
...
# Checks the version of Cyware with 4.3 version, where path is different.
...
function add_production_repository() {
    ...
    rpm --import https://packages.cyware.khulnasoft.com/key/GPG-KEY-CYWARE
    echo -e '[cyware]\ngpgcheck=1\ngpgkey=https://packages.cyware.khulnasoft.com/key/GPG-KEY-CYWARE\nenabled=1\nname=EL-$releasever - Cyware\nbaseurl=https://packages.cyware.khulnasoft.com/4.x/yum/\nprotect=1' | tee /etc/yum.repos.d/cyware.repo
