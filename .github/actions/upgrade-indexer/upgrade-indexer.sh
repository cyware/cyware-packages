...
echo "Installing old version of Cyware indexer..."
if [ ${sys_type} == "deb" ]; then
    apt-get -y install cyware-indexer
elif [ ${sys_type} == "rpm" ]; then
    add_production_repository
    yum -y install cyware-indexer
else
...
echo "Installing new version of Cyware indexer..."
if [ ${sys_type} == "deb" ]; then
