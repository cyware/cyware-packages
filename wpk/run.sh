...
main() {
    ...
    CYWARE_VERSION=$(cat src/VERSION)
    MAJOR=$(echo ${CYWARE_VERSION} | cut -dv -f2 | cut -d. -f1)
    MINOR=$(echo ${CYWARE_VERSION} | cut -d. -f2)
...
clean() {
    ...
    rm -rf src/{remoted,reportd,shared,unit_tests,cyware_db}

    # Clean syscheckd folder
    find src/syscheckd -type f -not -name "cyware-syscheckd" -not -name "libfimdb.dylib" -not -name "libfimdb.so" -delete
