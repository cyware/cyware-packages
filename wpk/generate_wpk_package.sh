...
# Program to build the Cyware WPK packages
...
function pack_wpk() {
    ...
    docker run -t --rm -v ${KEYDIR}:/etc/cyware:Z -v ${DESTINATION}:/var/local/cyware:Z -v ${PKG_PATH}:/var/pkg:Z \
...
function build_wpk_linux() {
    ...
    docker run -t --rm -v ${KEYDIR}:/etc/cyware:Z -v ${DESTINATION}:/var/local/cyware:Z \
        -v ${CHECKSUMDIR}:/var/local/checksum:Z \
