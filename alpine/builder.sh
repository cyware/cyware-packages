...
if [ "${local_source}" = "no" ]; then
    curl -sL https://github.com/cyware/cyware/tarball/${reference} | tar zx
fi
export version="$(cat cyware*/src/VERSION | cut -d 'v' -f 2)"

if [ "${future}" = "yes" ]; then
    old_version=$version
    MAJOR=$(echo $version | cut -dv -f2 | cut -d. -f1)
    export version="${MAJOR}.30.0"
    sed -i "s/${old_version}/${version}/g" "/cyware"*"/src/init/cyware-server.sh"
    sed -i "s/${old_version}/${version}/g" "/cyware"*"/src/init/cyware-client.sh"
    sed -i "s/${old_version}/${version}/g" "/cyware"*"/src/init/cyware-local.sh"
fi
...
if [ "${local_spec}" = "no" ]; then
    curl -sL https://github.com/cyware/cyware-packages/tarball/${spec_reference} | tar zx
    cp -r ./cyware*/alpine/SPECS/${target} ${spec_path}/
else
...
cd ${pkg_path} && sha512sum ${pkg_file} > /var/local/cyware/${pkg_file}.sha512
find ${pkg_path}/ -maxdepth 3 -type f -name "${file_name}*" -exec mv {} /var/local/cyware/ \;
