...
# Program to build the Cyware Virtual Machine
...
help () {
    echo -e ""
    echo -e "NAME"
    echo -e "$(basename "$0") - Build Cyware OVA."
    echo -e ""
    echo -e "SYNOPSIS"
...
clean() {
    ...
    cd "${scriptpath}"
    vagrant destroy -f
    OVA_VMDK="cyware-${OVA_VERSION}-disk001.vmdk"
...
build_ova() {
    ...
    OVA_VM="cyware-${OVA_VERSION}.ova"
    OVF_VM="cyware-${OVA_VERSION}.ovf"
    OVA_FIXED="cyware-${OVA_VERSION}-fixed.ova"
    ...
    # Get machine name
    VM_EXPORT=$(vboxmanage list vms | grep -i vm_cyware | cut -d "\"" -f2)

    # Create OVA with machine
    vboxmanage export "${VM_EXPORT}" -o "${OVA_VM}" \
    --vsys 0 \
    --product "Cyware v${OVA_VERSION} OVA" \
    --producturl "https://packages.cyware.khulnasoft.com/vm/cyware-${OVA_VERSION}.ova" \
    --vendor "Cyware, inc <info@khulnasoft.com>" --vendorurl "https://cyware.khulnasoft.com" \
    --version "$OVA_VERSION" --description "Cyware helps you to gain security visibility into your infrastructure by monitoring hosts at an operating system and application level. It provides the following capabilities: log analysis, file integrity monitoring, intrusions detection and policy and compliance monitoring." \
    || clean 1
...
main() {
    ...
    # Standarize OVA
    bash setOVADefault.sh "${scriptpath}" "${OUTPUT_DIR}/${OVA_VM}" "${OUTPUT_DIR}/${OVA_VM}" "${scriptpath}/cyware_ovf_template" "${OVA_VERSION}"  || clean 1
