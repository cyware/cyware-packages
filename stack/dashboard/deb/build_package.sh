...
clean() {
    ...
    # Clean the files
    rm -rf ${dockerfile_path}/{*.sh,*.tar.gz,cyware-*}
...
build_deb() {
    ...
    if [ "${build_base}" == "yes" ];then
        ...
        basefile="${outdir}/cyware-dashboard-base-${version}-${revision}-linux-x64.tar.xz"
        if ! test -f "${basefile}"; then
            echo "Did not find expected Cyware dashboard base file: ${basefile} in output path. Exiting..."
            exit 1
        fi
    fi
...
help() {
    ...
    echo "    --app-url <url>            [Optional] Set the repository from where the Cyware plugin should be downloaded. By default, will be used pre-release."
    echo "    -b, --build-base <yes/no>  [Optional] Build a new base or use a existing one. By default, yes."
    echo "    -r, --revision <rev>       [Optional] Package revision. By default: 1."
    echo "    -s, --store <path>         [Optional] Set the destination path of package. By default, an output folder will be created."
    echo "    --reference <ref>          [Optional] cyware-packages branch to download SPECs, not used by default."
