...
# Foundation.
Summary:     Cyware dashboard is a user interface and visualization tool for security-related data. Documentation can be found at https://documentation.cyware.khulnasoft.com/current/getting-started/components/cyware-dashboard.html
...
buildroot:   %{_tmppath}/%{name}-%{version}-%{release}-cyware-dashboard-%(%{__id_u} -n)
...
%global DASHBOARD_FILE cyware-dashboard-base-%{version}-%{release}-linux-x64.tar.xz
...
%description
Cyware dashboard is a user interface and visualization tool for security-related data. This Cyware central component enables exploring, visualizing, and analyzing the stored security alerts generated by the Cyware server. Cyware dashboard enables inspecting the status and managing the configurations of the Cyware cluster and agents as well as creating and managing users and roles. In addition, it allows testing the ruleset and making calls to the Cyware API. Documentation can be found at https://documentation.cyware.khulnasoft.com/current/getting-started/components/cyware-dashboard.html
...
sed -i 's/OSD_NODE_OPTS_PREFIX/OSD_PATH_CONF="\/etc\/cyware-dashboard" OSD_NODE_OPTS_PREFIX/g' "cyware-dashboard-base/bin/opensearch-dashboards"
sed -i 's/OSD_USE_NODE_JS_FILE_PATH/OSD_PATH_CONF="\/etc\/cyware-dashboard" OSD_USE_NODE_JS_FILE_PATH/g' "cyware-dashboard-base/bin/opensearch-dashboards-keystore"
...
cp cyware-dashboard-base/etc/node.options %{buildroot}%{CONFIG_DIR}
cp cyware-dashboard-base/etc/opensearch_dashboards.yml %{buildroot}%{CONFIG_DIR}
cp cyware-dashboard-base/VERSION %{buildroot}%{INSTALL_DIR}

mv cyware-dashboard-base/* %{buildroot}%{INSTALL_DIR}

# Set custom welcome styles

mkdir -p %{buildroot}%{INSTALL_DIR}/config

cp %{buildroot}%{INSTALL_DIR}/etc/services/cyware-dashboard.service %{buildroot}/etc/systemd/system/cyware-dashboard.service
cp %{buildroot}%{INSTALL_DIR}/etc/services/default %{buildroot}/etc/default/cyware-dashboard

chmod 640 %{buildroot}/etc/systemd/system/cyware-dashboard.service
chmod 640 %{buildroot}/etc/default/cyware-dashboard
...
chown root:root %{buildroot}/etc/systemd/system/cyware-dashboard.service

if [ "%{version}" = "99.99.0" ];then
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install https://packages-dev.cyware.khulnasoft.com/futures/ui/dashboard/cyware-99.99.0-%{release}.zip"
else
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install %{_url}"
fi

find %{buildroot}%{INSTALL_DIR}/plugins/cyware/ -exec chown %{USER}:%{GROUP} {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyware/ -type f -perm 644 -exec chmod 640 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyware/ -type f -perm 755 -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyware/ -type d -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyware/ -type f -perm 744 -exec chmod 740 {} \;

# -----------------------------------------------------------------------------

%pre
# Create the cyware-dashboard group if it doesn't exists
if [ $1 = 1 ]; then
  ...
  # Create the cyware-dashboard user if it doesn't exists
  if ! getent passwd %{USER} > /dev/null 2>&1; then
    useradd -g %{GROUP} -G %{USER} -d %{INSTALL_DIR}/ -r -s /sbin/nologin cyware-dashboard
  fi
fi
# Stop the services to upgrade the package
if [ $1 = 2 ]; then
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet cyware-dashboard > /dev/null 2>&1; then
    systemctl stop cyware-dashboard.service > /dev/null 2>&1
    touch %{INSTALL_DIR}/cyware-dashboard.restart
  # Check for SysV
  elif command -v service > /dev/null 2>&1 && service cyware-dashboard status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    service cyware-dashboard stop > /dev/null 2>&1
    touch %{INSTALL_DIR}/cyware-dashboard.restart
  fi
fi
...
%preun
if [ $1 = 0 ];then # Remove
  echo -n "Stopping cyware-dashboard service..."
...
if [ $1 = 0 ];then
  ...
  # Remove the cyware-dashboard user if it exists
  if getent passwd %{USER} > /dev/null 2>&1; then
    userdel %{USER} >/dev/null 2>&1
  fi
  # Remove the cyware-dashboard group if it exists
  if command -v getent > /dev/null 2>&1 && getent group %{GROUP} > /dev/null 2>&1; then
  ...
  # Remove /etc/cyware-dashboard and /usr/share/cyware-dashboard dirs
  rm -rf %{INSTALL_DIR}
  if [ -d %{PID_DIR} ]; then
    rm -rf %{PID_DIR}
  fi
...
if ! grep -q "/app/wz-home" %{CONFIG_DIR}/opensearch_dashboards.yml; then
  sed -i 's/\/app\/cyware/\/app\/wz-home/g' %{CONFIG_DIR}/opensearch_dashboards.yml
fi

if [ -f %{INSTALL_DIR}/cyware-dashboard.restart ]; then
  rm -f %{INSTALL_DIR}/cyware-dashboard.restart
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1; then
    systemctl restart cyware-dashboard.service > /dev/null 2>&1
  # Check for SysV
  elif command -v service > /dev/null 2>&1; then
    service cyware-dashboard restart > /dev/null 2>&1
  fi
...
%attr(750, %{USER}, %{GROUP}) "/etc/default/cyware-dashboard"
%attr(640, %{USER}, %{GROUP}) "%{CONFIG_DIR}/node.options"
%attr(640, root, root) "/etc/systemd/system/cyware-dashboard.service"
...
* Mon Apr 24 2023 support <info@khulnasoft.com> - 4.4.2
