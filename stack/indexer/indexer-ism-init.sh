#!/bin/bash
# Cyware Copyright (C) 2023 KhulnaSoft Ltd. (License GPLv2)
# Cyware - Indexer set rollover policy and templates

# Policy settings
MIN_SHARD_SIZE="25"
MIN_INDEX_AGE="7d"
MIN_DOC_COUNT="200000000"
ISM_INDEX_PATTERNS='["cyware-alerts-*", "cyware-archives-*", "-cyware-alerts-4.x-sample*"]'
ISM_PRIORITY="50"
...
function generate_rollover_policy() {
    cat <<-EOF
        {
            "policy": {
                "description": "Cyware rollover and alias policy",
...
function load_templates() {
    # Note: the cyware-template.json could also be loaded here.
...
function show_help() {
    echo -e ""
    echo -e "NAME"
    echo -e "        indexer-ism-init.sh - Manages the Index State Management plugin for Cyware indexer index rollovers policies."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "                Specifies the Cyware indexer hostname or IP."
    echo -e ""
    echo -e "        -p, --indexer-password <password>"
    echo -e "                Specifies the Cyware indexer admin user password."
    echo -e ""
...
function main() {
    ...
    # rollover policy
    aliases=("cyware-alerts" "cyware-archives")
    ...
    # Load the Cyware Indexer templates
