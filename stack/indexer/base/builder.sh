...
base_dir=/opt/cyware-indexer-base
...
sed -i 's|"$OPENSEARCH_HOME"/config|/etc/cyware-indexer|g' bin/opensearch-env
cp -r /root/stack/indexer/base/files/systemd-entrypoint bin/
mkdir -p ./etc/cyware-indexer/
cp -r ./config/* ./etc/cyware-indexer/
rm -rf ./config
cp -r /root/stack/indexer/base/files/etc/cyware-indexer/* ./etc/cyware-indexer/
...
# Base output
cd /opt
tar -Jcvf cyware-indexer-base-"${version}"-"${revision}"-linux-${architecture}.tar.xz cyware-indexer-base
cp cyware-indexer-base-"${version}"-"${revision}"-linux-${architecture}.tar.xz /tmp/output
