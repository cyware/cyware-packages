...
# Foundation.
Summary:     Cyware indexer is a search and analytics engine for security-related data. Documentation can be found at https://documentation.cyware.khulnasoft.com/current/getting-started/components/cyware-indexer.html
Name:        cyware-indexer
Version:     %{_version}
Release:     %{_release}
License:     GPL
...
%global INDEXER_FILE cyware-indexer-base-%{version}-%{release}-linux-x64.tar.xz

# -----------------------------------------------------------------------------

%description
Cyware indexer is a near real-time full-text search and analytics engine that gathers security-related data into one platform. This Cyware central component indexes and stores alerts generated by the Cyware server. Cyware indexer can be configured as a single-node or multi-node cluster, providing scalability and high availability. Documentation can be found at https://documentation.cyware.khulnasoft.com/current/getting-started/components/cyware-indexer.html
...
chown -R %{USER}:%{GROUP} cyware-indexer-*/*

# Copy base files into RPM_BUILD_ROOT directory
mv cyware-indexer-*/etc ${RPM_BUILD_ROOT}/
mv cyware-indexer-*%{SYS_DIR}/* ${RPM_BUILD_ROOT}%{SYS_DIR}/
rm -rf cyware-indexer-*/etc
rm -rf cyware-indexer-*/usr
cp -pr cyware-indexer-*/* ${RPM_BUILD_ROOT}%{INSTALL_DIR}/

# Build cyware-certs-tool
%{REPO_DIR}/builder.sh -c

# Build cyware-passwords-tool
%{REPO_DIR}/builder.sh -p

# Copy the security tools
cp %{REPO_DIR}/cyware-certs-tool.sh ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/
cp %{REPO_DIR}/cyware-passwords-tool.sh ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/
cp /root/documentation-templates/cyware/config.yml ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/config.yml

# Copy Cyware's config files for the security plugin
...
chmod 750 ${RPM_BUILD_ROOT}/etc/init.d/cyware-indexer
...
if [ $1 = 2 ]; then
  ...
  # Check for SysV
    ...
    touch %{INSTALL_DIR}/%{name}.restart
      ...
      if command -v invoke-rc.d >/dev/null && invoke-rc.d --quiet cyware-indexer status > /dev/null 2>&1; then
...
if [ $1 = 1 ];then # Install
    ...
    # To pick up /usr/lib/sysctl.d/cyware-indexer.conf
...
if [ -f %{INSTALL_DIR}/%{name}.restart ]; then
    echo -n "Starting cyware-indexer service..."
...
%attr(740, %{USER}, %{GROUP}) %{INSTALL_DIR}/plugins/opensearch-security/tools/cyware-certs-tool.sh
%attr(740, %{USER}, %{GROUP}) %{INSTALL_DIR}/plugins/opensearch-security/tools/cyware-passwords-tool.sh
