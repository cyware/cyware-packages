...
build_rpm() {
    ...
    if [ "${build_base}" == "yes" ];then
        ...
        basefile="${outdir}/cyware-indexer-base-${version}-${revision}-linux-x64.tar.xz"
        if ! test -f "${basefile}"; then
            echo "Did not find expected Cyware indexer base file: ${basefile} in output path. Exiting..."
            exit 1
        fi
    fi
