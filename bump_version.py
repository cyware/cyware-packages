"""
This script is used to bump the version of the Cyware packages repository.
    Copyright (C) 2015-2020, KhulnaSoft Ltd.
...
for spec_file in spec_files:
    with open(spec_file, 'r', encoding="utf-8") as file:
        ...
        REGEX=r'%changelog'
        changelog_string=(f"* {spec_date} support <info@khulnasoft.com> - {version}"
            "\n- More info: https://documentation.cyware.khulnasoft.com/current/release-"
...
for changelog_file in changelog_files:
    with open(changelog_file, 'r', encoding="utf-8") as file:
        ...
        filedata=file.read()
        install_type=re.search(r'(cyware-(agent|manager|indexer|dashboard))',
                               filedata).group(1)
        changelog_string=(f"{install_type} ({version}-RELEASE) stable; "
            "urgency=low\n\n  * More info: https://documentation.cyware.khulnasoft.com/"
            f"current/release-notes/release-{version.major}-{version.minor}-"
            f"{version.micro}.html\n\n -- "
            f"Cyware, Inc <info@khulnasoft.com>  {deb_changelog_date}\n\n")
        # Add new version to changelog
...
for copyrigth_file in copyright_files:
    with open(copyrigth_file, 'r', encoding="utf-8") as file:
        ...
        filedata=file.read()
        # Replace version and revision
        REGEX=(r'Cyware, Inc <info@khulnasoft.com> on '
                r'(\w+),\s(\d+)\s(\w+)\s(\d+)\s(\d+):(\d+):(\d+)\s\+(\d+)')
        filedata=re.sub(REGEX,
                    f"Cyware, Inc <info@khulnasoft.com> on {deb_changelog_date}",
                    filedata)
...
for pkgproj_file in pkgproj_files:
    with open(pkgproj_file, 'r', encoding="utf-8") as file:
        ...
        filedata=re.sub(REGEX, f'<string>{version}-{args.revision}</string>',
                          filedata)
        REGEX=r'<string>cyware-agent-(\d+\.\d+\.\d+)-(\d+)'
        filedata=re.sub(REGEX,
                    f'<string>cyware-agent-{version}-{args.revision}',
                    filedata)
...
for test_file in test_files:
    with open(test_file, 'r', encoding="utf-8") as file:
        ...
        filedata=file.read()
        # Replace version and revision
        REGEX=r'cyware-manager.x86_64\s+(\d+\.\d+\.\d+)-(\d+)'
        filedata=re.sub(REGEX,
                    f'cyware-manager.x86_64 {version}-{args.revision}',
                    filedata)
        REGEX=r'cyware_version=\"(\d+\.\d+\.\d+)\"'
        filedata=re.sub(REGEX, f'cyware_version=\"{version}\"', filedata)
...
for install_variables_file in install_variables_files:
    with open(install_variables_file, 'r', encoding="utf-8") as file:
        ...
        filedata=file.read()
        # Replace version and revision
        REGEX=r'cyware_major=\"(\d+\.\d+)\"'
        filedata=re.sub(REGEX,
                    f'cyware_major=\"{version.major}.{version.minor}\"',
                    filedata)
        REGEX=r'cyware_version=\"(\d+\.\d+\.\d+)\"'
        filedata=re.sub(REGEX, f'cyware_version=\"{version}\"', filedata)
...
for changelog_md_file in changelog_md_files:
    with open(changelog_md_file, 'r', encoding="utf-8") as file:
        ...
        changelog_string=(f"## [{version}]\n\n- https://github.com/cyware/"
                          f"cyware-packages/releases/tag/v{version}\n")
