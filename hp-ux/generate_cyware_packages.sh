...
# Cyware HP-UX Package builder.


install_path="/var/ossec"
current_path=`pwd`
build_tools_path="/home/okkam"
source_directory=${current_path}/cyware-sources
...
# Needed variables to build Cyware with custom GCC and cmake
...
build_environment() {
    ...
    curl -k -SO http://packages.cyware.khulnasoft.com/utils/gcc/gcc_9.4_HPUX_build.tar.gz
    ...
    cd ${build_tools_path}
    curl -k -SO http://packages.cyware.khulnasoft.com/utils/cmake/cmake_3.22.2_HPUX_build.tar.gz
...
compute_version_revision() {
    cyware_version=$(cat ${source_directory}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    echo ${cyware_version} > /tmp/VERSION
    echo ${cyware_revision} > /tmp/REVISION
...
download_source() {
    ...
    /usr/local/bin/curl -k -L -O https://github.com/cyware/cyware/archive/${cyware_branch}.zip
    /usr/local/bin/unzip ${cyware_branch}.zip
    mv cyware-* ${source_directory}
    compute_version_revision
}

check_version() {
    cyware_version=`cat ${source_directory}/src/VERSION`
...
compile() {
    echo "Compiling code"
    # Compile and install cyware
    cd ${source_directory}/src
    config
    check_version
    gmake deps RESOURCES_URL=http://packages.cyware.khulnasoft.com/deps/${deps_version} TARGET=agent
...
create_package() {
    ...
    cyware_version=`echo "${cyware_version}" | cut -d v -f 2`
    pkg_name="cyware-agent-${cyware_version}-${cyware_revision}-hpux-11v3-ia64.tar"
    tar cvpf ${target_dir}/${pkg_name} ${install_path} /sbin/init.d/cyware-agent /sbin/rc2.d/S97cyware-agent /sbin/rc3.d/S97cyware-agent
...
clean() {
    ...
    find /sbin -name "*cyware-agent*" -exec rm {} \;
    userdel cyware
    groupdel cyware
