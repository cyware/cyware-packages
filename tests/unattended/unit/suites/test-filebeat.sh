...
@setup-test {
    ...
    cyware_major="4.3"
    filebeat_cyware_template="https://raw.githubusercontent.com/cyware/cyware/${cyware_major}/extensions/elasticsearch/7.x/cyware-template.json"
    repobaseurl="https://packages.cyware.khulnasoft.com/4.x"
    filebeat_cyware_module="${repobaseurl}/filebeat/cyware-filebeat-0.1.tar.gz"
}
...
test-03-filebeat_copyCertificates-distributed-assert() {
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./cyware1.pem
    mv /etc/filebeat/certs/cyware1.pem /etc/filebeat/certs/filebeat.pem
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./cyware1-key.pem
    mv /etc/filebeat/certs/cyware1-key.pem /etc/filebeat/certs/filebeat-key.pem
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./root-ca.pem
}
...
test-08-filebeat_configure-no-previous-variables() {
    ...
    filebeat_cyware_template=""
    filebeat_cyware_module=""
...
test-08-filebeat_configure-no-previous-variables-assert() {
    curl -so /etc/filebeat/cyware-template.json --max-time 300
...
test-09-filebeat_configure-one-elastic-node() {
    load-filebeat_configure
    @mocktrue curl -s ${filebeat_cyware_module} --max-time 300
    @mock tar -xvz -C /usr/share/filebeat/module
    @mocktrue echo admin
...
test-09-filebeat_configure-one-elastic-node-assert() {
    curl -so /etc/filebeat/cyware-template.json https://raw.githubusercontent.com/cyware/cyware/4.3/extensions/elasticsearch/7.x/cyware-template.json --max-time 300
    chmod go+r /etc/filebeat/cyware-template.json
    installCommon_getConfig filebeat/filebeat_distributed.yml /etc/filebeat/filebeat.yml
    echo "output.elasticsearch.hosts:" >> /etc/filebeat/filebeat.yml
