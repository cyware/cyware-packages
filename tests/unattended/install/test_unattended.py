...
def read_services():
    services = None
    p = Popen(['/var/ossec/bin/cyware-control', 'status'], stdin=PIPE, stdout=PIPE, stderr=PIPE)
...
def get_password(username):
    ...
    with tarfile.open("../../../unattended_installer/cyware-install-files.tar") as configurations:
        configurations.extract("cyware-install-files/cyware-passwords.txt")

    with open("cyware-install-files/cyware-passwords.txt", 'r') as pass_file:
...
def get_cyware_version():
    cyware_version = None
    cyware_version = subprocess.getoutput('/var/ossec/bin/cyware-control info | grep VERSION | cut -d "=" -f2 | sed s/\\"//g')
    return cyware_version

def get_indexer_ip():

    with open("/etc/cyware-indexer/opensearch.yml", 'r') as stream:
        dictionary = yaml.safe_load(stream)
    return (dictionary.get('network.host'))

def get_dashboard_ip():

    with open("/etc/cyware-dashboard/opensearch_dashboards.yml", 'r') as stream:
        dictionary = yaml.safe_load(stream)
...
def api_call_indexer(host,query,address,api_protocol,api_user,api_pass,api_port):
    ...
    if (query == ""):   # Calling ES API without query
        ...
        if (api_pass != "" and api_pass != ""):
            resp = requests.post(api_protocol + '://' + address + ':' + api_port + "/cyware-alerts-4.x-*/_search",
                        json=query,
                        auth=(api_user,
                        api_pass),
                        verify=False)
...
def get_cyware_api_status():

    protocol = 'https'
    host = get_api_ip()
    port = 55000
    user = 'cyware'
    password = get_password('cyware')
...
@pytest.mark.cyware
def test_check_cyware_manager_authd():
    assert check_call("ps -xa | grep cyware-authd | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_db():
    assert check_call("ps -xa | grep cyware-db | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_execd():
    assert check_call("ps -xa | grep cyware-execd | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_analysisd():
    assert check_call("ps -xa | grep cyware-analysisd | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_syscheckd():
    assert check_call("ps -xa | grep cyware-syscheckd | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_remoted():
    assert check_call("ps -xa | grep cyware-remoted | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_logcollec():
    assert check_call("ps -xa | grep cyware-logcollec | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_monitord():
    assert check_call("ps -xa | grep cyware-monitord | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_modulesd():
    assert check_call("ps -xa | grep cyware-modulesd | grep -v grep", shell=True) != ""

@pytest.mark.cyware
def test_check_cyware_manager_apid():
    assert check_call("ps -xa | grep cyware-apid | grep -v grep", shell=True) != ""

@pytest.mark.cyware_cluster
def test_check_cyware_manager_clusterd():
...
def test_check_indexer_process():
    assert check_call("ps -xa | grep cyware-indexer | grep -v grep | cut -d \" \" -f15", shell=True) != ""

@pytest.mark.dashboard
def test_check_dashboard_process():
    assert check_call("ps -xa | grep cyware-dashboard | grep -v grep", shell=True) != ""
...
@pytest.mark.cyware
def test_check_cyware_api_status():
    assert get_cyware_api_status() == "Cyware API REST"
