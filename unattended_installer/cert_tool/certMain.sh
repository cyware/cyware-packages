...
function getHelp() {
    ...
    echo -e ""
    echo -e "NAME"
    echo -e "        cyware-cert-tool.sh - Manages the creation of certificates of the Cyware components."
    echo -e ""
    echo -e "SYNOPSIS"
    echo -e "        cyware-cert-tool.sh [OPTIONS]"
    echo -e ""
    echo -e "DESCRIPTION"
    ...
    echo -e ""
    echo -e "        -wd,  --cyware-dashboard-certificates </path/to/root-ca.pem> </path/to/root-ca.key>"
    echo -e "                Creates the Cyware dashboard certificates, add root-ca.pem and root-ca.key."
    echo -e ""
    echo -e "        -wi,  --cyware-indexer-certificates </path/to/root-ca.pem> </path/to/root-ca.key>"
    echo -e "                Creates the Cyware indexer certificates, add root-ca.pem and root-ca.key."
    echo -e ""
    echo -e "        -ws,  --cyware-server-certificates </path/to/root-ca.pem> </path/to/root-ca.key>"
    echo -e "                Creates the Cyware server certificates, add root-ca.pem and root-ca.key."
    echo -e ""
    echo -e "        -tmp,  --cert_tmp_path </path/to/tmp_dir>"
    echo -e "                Modifies the default tmp directory (/tmp/cyware-ceritificates) to the specified one."
    ...
    if [ -n "${1}" ]; then
        ...
        do
            ...
            "-v"|"--verbose")
                ...
                shift 1
                ;;
            "-wd"|"--cyware-dashboard-certificates")
                if [[ -z "${2}" || -z "${3}" ]]; then
                    common_logger -e "Error on arguments. Probably missing </path/to/root-ca.pem> </path/to/root-ca.key> after -wd|--cyware-dashboard-certificates"
                    getHelp
                    exit 1
                else
                    cdashboard=1
                ...
                ;;
            "-wi"|"--cyware-indexer-certificates")
                if [[ -z "${2}" || -z "${3}" ]]; then
                    common_logger -e "Error on arguments. Probably missing </path/to/root-ca.pem> </path/to/root-ca.key> after -wi|--cyware-indexer-certificates"
                    getHelp
                    exit 1
                else
                    cindexer=1
                ...
                ;;
            "-ws"|"--cyware-server-certificates")
                if [[ -z "${2}" || -z "${3}" ]]; then
                    common_logger -e "Error on arguments. Probably missing </path/to/root-ca.pem> </path/to/root-ca.key> after -ws|--cyware-server-certificates"
                    getHelp
                    exit 1
                else
                    cserver=1
        ...
        if [[ -d "${base_path}"/cyware-certificates ]]; then
            if [ -n "$(ls -A "${base_path}"/cyware-certificates)" ]; then
                common_logger -e "Directory cyware-certificates already exists in the same path as the script. Please, remove the certs directory to create new certificates."
                exit 1
            fi
        fi
        ...
        if [[ -n "${cadmin}" ]]; then
            ...
            cert_cleanFiles
            cert_setpermisions
            eval "mv ${cert_tmp_path} ${base_path}/cyware-certificates ${debug}"
        fi
        ...
        if [[ -n "${all}" ]]; then
            ...
            if cert_generateIndexercertificates; then
                common_logger "Cyware indexer certificates created."
            fi
            if cert_generateFilebeatcertificates; then
                common_logger "Cyware server certificates created."
            fi
            if cert_generateDashboardcertificates; then
                common_logger "Cyware dashboard certificates created."
            fi
            cert_cleanFiles
            cert_setpermisions
            eval "mv ${cert_tmp_path} ${base_path}/cyware-certificates ${debug}"
