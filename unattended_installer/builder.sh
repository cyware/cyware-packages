...
# Tool to create cyware-install.sh, cyware-cert-tool.sh
# and cyware-passwords-tool.sh
...
function getHelp() {
    ...
    echo -e "DESCRIPTION"
    echo -e "        -i,  --installer"
    echo -e "                Builds the unattended installer single file cyware-install.sh"
    echo -e ""
    echo -e "        -c,  --cert-tool"
    echo -e "                Builds the certificate creation tool cyware-cert-tool.sh"
    echo -e ""
    ...
    echo -e "                Builds the password creation and modification tool cyware-password-tool.sh"
    echo -e ""
    echo -e "        -h,  --help"
    echo -e "                Shows help."
    exit 1
...
function buildInstaller() {
    ...
    output_script_path="${base_path_builder}/cyware-install.sh"
...
# Foundation." >> "${output_script_path}"
    ...
    if [ -n "${development}" ]; then
        ...
        echo 'readonly repogpg="https://packages-dev.cyware.khulnasoft.com/key/GPG-KEY-CYWARE"' >> "${output_script_path}"
        echo 'readonly repobaseurl="https://packages-dev.cyware.khulnasoft.com/'${devrepo}'"' >> "${output_script_path}"
        echo 'readonly reporelease="unstable"' >> "${output_script_path}"
        echo 'readonly filebeat_cyware_module="${repobaseurl}/filebeat/cyware-filebeat-0.2.tar.gz"' >> "${output_script_path}"
        echo 'readonly bucket="packages-dev.cyware.khulnasoft.com"' >> "${output_script_path}"
        echo 'readonly repository="'"${devrepo}"'"' >> "${output_script_path}"
        sed -i 's|v${cyware_version}|${cyware_version}|g' "${resources_installer}/installVariables.sh"
    else
        echo 'readonly repogpg="https://packages.cyware.khulnasoft.com/key/GPG-KEY-CYWARE"' >> "${output_script_path}"
        echo 'readonly repobaseurl="https://packages.cyware.khulnasoft.com/4.x"' >> "${output_script_path}"
        echo 'readonly reporelease="stable"' >> "${output_script_path}"
        echo 'readonly filebeat_cyware_module="${repobaseurl}/filebeat/cyware-filebeat-0.2.tar.gz"' >> "${output_script_path}"
        echo 'readonly bucket="packages.cyware.khulnasoft.com"' >> "${output_script_path}"
    ...
    ## JAVA_HOME
    echo "export JAVA_HOME=\"/usr/share/cyware-indexer/jdk/\"" >> "${output_script_path}"
    ...
    curl -s "https://raw.githubusercontent.com/cyware/cyware/${source_branch}/src/init/dist-detect.sh" | sed '/^#/d' >> "${output_script_path}"
    echo "}" >> "${output_script_path}"
...
function buildPasswordsTool() {
    output_script_path="${base_path_builder}/cyware-passwords-tool.sh"
...
function buildCertsTool() {
    output_script_path="${base_path_builder}/cyware-certs-tool.sh"
...
function builder_main() {
    ...
    if [ -n "${installer}" ]; then
        ...
        if [ -n "${change_filebeat_url}" ]; then
            sed -i -E "s|(https.+)master(.+cyware-template.json)|\1\\$\\{source_branch\\}\2|"  "${resources_installer}/installVariables.sh"
        fi
        if [ -n "${development}" ]; then
            sed -i 's|${cyware_version}|v${cyware_version}|g' "${resources_installer}/installVariables.sh"
        fi
    fi
...
function checkDistDetectURL() {
    ...
    retries=0
    eval "curl -s -o /dev/null 'https://raw.githubusercontent.com/cyware/cyware/${source_branch}/src/init/dist-detect.sh' --retry 5 --retry-delay 5 --max-time 300 --fail"
    e_code="${PIPESTATUS[0]}"
    while [ "${e_code}" -eq 7 ] && [ "${retries}" -ne 12 ]; do
        retries=$((retries+1))
        sleep 5
        eval "curl -s -o /dev/null 'https://raw.githubusercontent.com/cyware/cyware/${source_branch}/src/init/dist-detect.sh' --fail"
        e_code="${PIPESTATUS[0]}"
    done
...
function checkFilebeatURL() {
    ...
    # Import variables
    eval "$(grep -E "filebeat_cyware_template=" "${resources_installer}/installVariables.sh")"
    new_filebeat_url="https://raw.githubusercontent.com/cyware/cyware/master/extensions/elasticsearch/7.x/cyware-template.json"

    # Get the response of the URL and check it
    response=$(curl -I --write-out '%{http_code}' --silent --output /dev/null $filebeat_cyware_template)
    if [ "${response}" != "200" ]; then
        ...
        if [ "${response}" != "200" ]; then
            echo -e "Error: Could not get the Filebeat Cyware template. "
        # If matches, replace the variable of installVariables to the new one
        else
            echo -e "Changing Filebeat URL..."
            sed -i -E "s|filebeat_cyware_template=.*|filebeat_cyware_template=\"${new_filebeat_url}\"|g" "${resources_installer}/installVariables.sh"
