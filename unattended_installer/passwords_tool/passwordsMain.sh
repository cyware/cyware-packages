...
function getHelp() {
    ...
    echo -e ""
    echo -e "NAME"
    echo -e "        $(basename "${0}") - Manage passwords for Cyware indexer users."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "DESCRIPTION"
    echo -e "        -a,  --change-all"
    echo -e "                Changes all the Cyware indexer and Cyware API user passwords and prints them on screen."
    echo -e "                To change API passwords -au|--admin-user and -ap|--admin-password are required."
    echo -e ""
    echo -e "        -A,  --api"
    echo -e "                Change the Cyware API password."
    echo -e "                Requires -u|--user, and -p|--password, -au|--admin-user and -ap|--admin-password."
    echo -e ""
    echo -e "        -au,  --admin-user <adminUser>"
    echo -e "                Admin user for Cyware API, Required to change Cyware API passwords."
    echo -e ""
    echo -e "        -ap,  --admin-password <adminPassword>"
    echo -e "                Password for Cyware API admin user, Required to change Cyware API passwords."
    echo -e ""
    echo -e "        -u,  --user <user>"
    ...
    echo -e ""
    echo -e "        -f,  --file <cyware-passwords.txt>"
    echo -e "                Changes the passwords for the ones given in the file."
    echo -e ""
    echo -e "                Cyware indexer users must have this format:"
    echo -e ""
    echo -e "                    # Description"
    echo -e "                      indexer_username: <user>"
    echo -e "                      indexer_password: <password>"
    echo -e ""
    echo -e "                Cyware API users must have this format:"
    echo -e ""
    echo -e "                    # Description"
    echo -e "                      api_username: <user>"
    echo -e "                      api_password: <password>"
    echo -e ""
    echo -e "        -gf, --generate-file <cyware-passwords.txt>"
...
function main() {
    ...
    if [ -n "${1}" ]; then
        ...
        export JAVA_HOME=/usr/share/cyware-indexer/jdk/
        ...
        if [ -n "${changeall}" ] || [ -n "${p_file}" ]; then
            ...
            if [ -n "${adminUser}" ] && [ -n "${adminPassword}" ]; then
                ...
                common_logger "Cyware API admin credentials not provided, Cyware API passwords not changed."
            fi
            if [ -n "${changeall}" ]; then
                passwords_generatePassword
            fi
