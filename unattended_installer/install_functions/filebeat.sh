# Cyware installer - filebeat.sh functions.
...
function filebeat_configure(){
    ...
    eval "common_curl -sSo /etc/filebeat/cyware-template.json ${filebeat_cyware_template} --max-time 300 --retry 5 --retry-delay 5 --fail"
    if [ ! -f "/etc/filebeat/cyware-template.json" ]; then
        common_logger -e "Error downloading cyware-template.json file."
        installCommon_rollBack
        exit 1
    fi
    common_logger -d "Filebeat template was download successfully."

    eval "chmod go+r /etc/filebeat/cyware-template.json ${debug}"
    eval "(common_curl -sS ${filebeat_cyware_module} --max-time 300 --retry 5 --retry-delay 5 --fail | tar -xvz -C /usr/share/filebeat/module) ${debug}"
    if [ ! -d "/usr/share/filebeat/module" ]; then
        common_logger -e "Error downloading cyware filebeat module."
        installCommon_rollBack
        exit 1
    fi
...
function filebeat_copyCertificates() {
    ...
    if [ -f "${tar_file}" ]; then
        if [ -n "${AIO}" ]; then
            ...
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} --wildcards cyware-install-files/${server_node_names[0]}.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} --wildcards cyware-install-files/${server_node_names[0]}-key.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} cyware-install-files/root-ca.pem --strip-components 1 ${debug}"
            eval "rm -rf ${filebeat_cert_path}/cyware-install-files/ ${debug}"
        else
            ...
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} cyware-install-files/${winame}.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} cyware-install-files/${winame}-key.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} cyware-install-files/root-ca.pem --strip-components 1 ${debug}"
            eval "rm -rf ${filebeat_cert_path}/cyware-install-files/ ${debug}"
        fi
