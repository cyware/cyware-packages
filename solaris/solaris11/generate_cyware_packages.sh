...
# Cyware Solaris 11 Package builder.

REPOSITORY="https://github.com/cyware/cyware"
cyware_branch="master"
...
set_control_binary() {
    if [ -e ${SOURCE}/src/VERSION ]; then
        cyware_version=`cat ${SOURCE}/src/VERSION`
        number_version=`echo "${cyware_version}" | cut -d v -f 2`
...
build_environment() {
    ...
    curl -L http://packages.cyware.khulnasoft.com/utils/gcc/gcc-5.5.0.tar.gz | gtar xz
    cd gcc-5.5.0
    curl -L http://packages.cyware.khulnasoft.com/utils/gcc/mpfr-2.4.2.tar.bz2 | gtar xj
    mv mpfr-2.4.2 mpfr
    curl -L http://packages.cyware.khulnasoft.com/utils/gcc/gmp-4.3.2.tar.bz2 | gtar xj
    mv gmp-4.3.2 gmp
    curl -L http://packages.cyware.khulnasoft.com/utils/gcc/mpc-0.8.1.tar.gz | gtar xz
    mv mpc-0.8.1 mpc
    curl -L http://packages.cyware.khulnasoft.com/utils/gcc/isl-0.14.tar.bz2 | gtar xj
    mv isl-0.14 isl
    ...
    curl -sL http://packages.cyware.khulnasoft.com/utils/cmake/cmake-3.18.3.tar.gz | gtar xz
    cd cmake-3.18.3
    ./bootstrap
    gmake -j$(nproc) && gmake install
...
compute_version_revision() {
    cyware_version=$(cat ${SOURCE}/src/VERSION | cut -d "-" -f1 | cut -c 2-)
    revision="$(cat ${SOURCE}/src/REVISION)"

    echo $cyware_version > /tmp/VERSION
...
download_source() {
    ...
    if [[ "${cyware_branch}" != "trunk" ]] || [[ "${cyware_branch}" != "master" ]]; then
        cd $SOURCE
        git checkout $cyware_branch
    fi
    cd ${current_path}
    compute_version_revision
}
...
#Compile and install cyware-agent
compile() {
...
create_package() {
    ...
    if [ $(echo $VERSION | grep "v") ]; then
        ...
        sed "s/<VERSION>/$ver/" ${current_path}/cyware-agent.mog-template > ${current_path}/cyware-agent.mog-aux
    else
        sed "s/<VERSION>/$VERSION/" ${current_path}/cyware-agent.mog-template > ${current_path}/cyware-agent.mog-aux
    fi
    sed "s/<TAG>/$VERSION/" ${current_path}/cyware-agent.mog-aux > ${current_path}/cyware-agent.mog

    echo "Building the package cyware-agent_$VERSION-sol11-${arch}.p5p"

    set_control_binary


    # Package generation process
    pkgsend generate ${install_path} | pkgfmt > cyware-agent.p5m.1
    ...
    python solaris_fix.py -t SPECS/template_agent.json -p cyware-agent.p5m.1 # Fix p5m.1 file
    mv cyware-agent.p5m.1.aux.fixed cyware-agent.p5m.1
    ...
    for file in etc/ossec.conf etc/local_internal_options.conf etc/client.keys; do
        sed "s:file $file.*:& preserve=install-only:"  cyware-agent.p5m.1 > cyware-agent.p5m.1.aux_sed
        mv cyware-agent.p5m.1.aux_sed cyware-agent.p5m.1
    done
    # Add service files
    echo "file smf_manifest.xml path=lib/svc/manifest/site/post-install.xml owner=root group=sys mode=0744 restart_fmri=svc:/system/manifest-import:default" >> cyware-agent.p5m.1
    echo "dir  path=var/ossec/installation_scripts owner=root group=bin mode=0755" >> cyware-agent.p5m.1
    echo "file postinstall.sh path=var/ossec/installation_scripts/postinstall.sh owner=root group=bin mode=0744" >> cyware-agent.p5m.1
    echo "file cyware-agent path=etc/init.d/cyware-agent owner=root group=sys mode=0744" >> cyware-agent.p5m.1
    echo "file S97cyware-agent path=etc/rc2.d/S97cyware-agent owner=root group=sys mode=0744" >> cyware-agent.p5m.1
    echo "file S97cyware-agent path=etc/rc3.d/S97cyware-agent owner=root group=sys mode=0744" >> cyware-agent.p5m.1

    # Add user and group cyware
    echo "group groupname=cyware" >> cyware-agent.p5m.1
    echo "user username=cyware group=cyware" >> cyware-agent.p5m.1

    # Necessary to upgrade from < 4.3 versions
    echo "group groupname=ossec" >> cyware-agent.p5m.1
    echo "user username=ossec group=ossec" >> cyware-agent.p5m.1

    pkgmogrify -DARCH=`uname -p` cyware-agent.p5m.1 cyware-agent.mog | pkgfmt > cyware-agent.p5m.2
    pkgsend -s http://localhost:9001 publish -d ${install_path} -d /etc/init.d -d /etc/rc2.d -d /etc/rc3.d -d ${current_path} cyware-agent.p5m.2 > pack
    package=`cat pack | grep cyware | cut -c 13-` # This extracts the name of the package generated in the previous step
    rm -f *.p5p
    pkg_name="cyware-agent_$VERSION-sol11-${arch}.p5p"
...
create_repo() {
    zfs create rpool/cyware
    zfs set mountpoint=/cyware rpool/cyware
    pkgrepo create /cyware
    ls /cyware
    pkgrepo -s /cyware set publisher/prefix=cyware
    svccfg -s application/pkg/server setprop \ pkg/inst_root=/cyware
...
uninstall() {
    ...
    rm -f `find /etc | grep cyware`
    rm -f /etc/rc3.d/S97cyware-agent
    rm -f /etc/rc2.d/S97cyware-agent
}
...
clean() {
    ...
    umount /cyware
    zfs destroy rpool/cyware
    rm -rf /cyware
    rm -rf $SOURCE/cyware
    rm -f cyware-agent.p5m*
    rm -f cyware-agent.mog
    rm -f cyware-agent.mog-aux
    rm -f pack
}
...
show_help() {
    ...
    echo -e "                Select Git branch or tag e.g. ${cyware_branch}."
    echo -e ""
    echo -e "        -c, --checksum"
