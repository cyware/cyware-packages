...
# Cyware Solaris 10 Package builder.


# CONFIGURATION VARIABLES
cyware_branch="$(echo "$2" | cut -d "/" -f2)"
...
package(){
    cd ${CURRENT_PATH}
    find ${install_path} | awk 'length > 0' > "cyware-agent_$VERSION.list"
    ...
    echo "i pkginfo=pkginfo" > "cyware-agent_$VERSION.proto"
    echo "i checkinstall=checkinstall.sh" >> "cyware-agent_$VERSION.proto"
    echo "i preinstall=preinstall.sh" >> "cyware-agent_$VERSION.proto"
    echo "i postinstall=postinstall.sh" >> "cyware-agent_$VERSION.proto"
    echo "i preremove=preremove.sh" >> "cyware-agent_$VERSION.proto"
    echo "i postremove=postremove.sh" >> "cyware-agent_$VERSION.proto"
    echo "f none /etc/init.d/cyware-agent  0755 root root" >> "cyware-agent_$VERSION.proto"
    echo "s none /etc/rc2.d/S97cyware-agent=/etc/init.d/cyware-agent" >> "cyware-agent_$VERSION.proto"
    echo "s none /etc/rc3.d/S97cyware-agent=/etc/init.d/cyware-agent" >> "cyware-agent_$VERSION.proto"
    cat "cyware-agent_$VERSION.list" | pkgproto >> "cyware-agent_$VERSION.proto"

    echo $VERSION
    pkgmk -o -r / -d . -f "cyware-agent_$VERSION.proto"
    pkg_name="cyware-agent_$VERSION-sol10-$ARCH.pkg"
    pkgtrans -s ${CURRENT_PATH} "${pkg_name}" cyware-agent
...
clean(){
    ...
    cd ${CURRENT_PATH}
    rm -rf ${SOURCE}
    rm -rf cyware-agent cyware *.list *proto
    rm -f *.new
