...
TAR_URL="https://packages-dev.cyware.khulnasoft.com/utils/centos-5-i386-build/centos-5-i386.tar.gz"
...
clean() {
    ...
    # Clean the files
    rm -rf ${DOCKERFILE_PATH}/{*.tar.gz,cyware*} ${DOCKERFILE_PATH}/build.sh ${SOURCES_DIRECTORY}
...
build_rpm() {
    ...
    if [ ! -z "${LOCAL_SOURCE_CODE}" ]; then
        CUSTOM_CODE_VOL="-v ${LOCAL_SOURCE_CODE}:/cyware-local-src:Z"
    ...
    docker run -t --rm -v ${OUTDIR}:/var/local/cyware:Z \
...
help() {
    ...
    echo "    --sources <path>             [Optional] Absolute path containing cyware source code. This option will use local source code instead of downloading it from GitHub."
    echo "    --packages-branch <branch>   [Optional] Select Git branch or tag from cyware-packages repository. e.g ${PACKAGES_BRANCH}"
