...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows Cyware agent msi package.

    PARAMETERS TO BUILD CYWARE-AGENT MSI:
    ...
    USAGE:
        ...
        * CYWARE:
          $ ./generate_cyware_msi.ps1  -OPTIONAL_REVISION {{ REVISION }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}

            Build a devel msi:    $ ./generate_cyware_msi.ps1 -OPTIONAL_REVISION 2 -SIGN no
            Build a prod msi:     $ ./generate_cyware_msi.ps1 -OPTIONAL_REVISION 1 -SIGN yes -
    "
    Exit
}
...
function ComputeMsiName() {
    ...
    $MSI_NAME="cyware-agent-$VERSION-$OPTIONAL_REVISION.msi"
    return $MSI_NAME
}

function BuildCywareMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        & $SIGNTOOL_EXE sign /a /tr http://timestamp.digicert.com /fd SHA256 /td SHA256 "..\cyware_modules\syscollector\build\bin\syscollector.dll"
        & $SIGNTOOL_EXE sign /a /tr http://timestamp.digicert.com /fd SHA256 /td SHA256 "..\syscheckd\build\bin\libfimdb.dll"
    }

    Write-Host "Building MSI installer..."

    & $CANDLE_EXE -nologo .\cyware-installer.wxs -out "cyware-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\cyware-installer.wixobj" -out $MSI_NAME  -ext WixUtilExtension -ext WixUiExtension
